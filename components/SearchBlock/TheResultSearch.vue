<template>
  <div v-for="(wordResult, index) in searchResults" :key="wordResult + index" class="background-effect result-words">
    <div class="result-words__text ">
      <div class="result-words__original">
        {{wordResult.original_word}}
        <UITheSound v-if="wordResult.sound_file" :sound-file="wordResult.sound_file" />
      </div>
      <span>—</span>
      <div class="result-words__translate">
        {{wordResult.translate_word.join(', ')}}
      </div>
    </div>

    <div class="result-words__redirect">
      {{$t('translate.redirect')}}
